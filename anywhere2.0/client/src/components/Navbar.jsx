  ...
  return (
    <nav className="bg-gradient-to-r from-blue-700 to-blue-500 p-4 backdrop-blur-lg bg-opacity-40 border border-blue-300 border-opacity-20">
      <div className="container mx-auto flex flex-col md:flex-row justify-between items-center">
        <Link to="/home" className="text-white font-bold text-xl flex items-center mb-4 md:mb-0">
        ...
        <div className="flex items-center space-x-4">
          {menu && (
            <div className="sm:flex sm:flex-col gap-3">
              ...
              {user.username ? (
                <button onClick={handleLogout} className="bg-red-800 text-red-100 rounded px-4 py-2 transition-all duration-200 hover:bg-red-500 hover:text-white">
                  <FaUser className="inline-block mr-1" /> Logout
                </button>
              ) : (
                <button onClick={handleLogin} className="bg-red-800 text-red-100 rounded px-4 py-2 transition-all duration-200 hover:bg-red-500 hover:text-white">
                  <FaUser className="inline-block mr-1" /> Login
                </button>
              )}
            </div>
          )}
          <BsMenuButtonWideFill className="text-2xl text-white" onClick={handleMenu} />
        </div>
      </div>
    </nav>
  );
